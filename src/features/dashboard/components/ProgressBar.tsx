...
export default function ProgressBar({ current, target, label, period }: ProgressBarProps) {
  ...
  return (
    <div className={`modern-card p-6 transition-all duration-500 ${isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-4'}`}>
      ...
      <div className="mt-3 text-center">
        <span className={`text-sm font-semibold ${isPlanCompleted ? 'text-green-600 dark:text-green-400' : 'text-gray-600 dark:text-gray-300'}`}>
          ...
          {[1, 2, 3].map((i) => (
            <div 
              ...
              className={`w-2 h-2 rounded-full transition-all duration-500 ${
                animatedProgress > (i * 33) ? 'bg-gradient-to-r from-blue-500 to-indigo-500 shadow-lg' : 'bg-gray-300 dark:bg-gray-600'
              }`}
            />
          ))}
        </div>
